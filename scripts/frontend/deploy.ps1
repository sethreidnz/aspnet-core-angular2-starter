...
#Usage: ./deploy/deploy.ps1 -FrontendProjectName FrontendProjectName -BuildSourcesDirectory buildDirectory -WebAppName webappName -WebSlotName webslotName -ResourceGroup resourceGroupName -StorageAccountName storageContainer  -StorageAccountKey storageKey -StorageContainerName containerName -MainHashSettingString settingsString  -VendorHashSettingString settingsString  -PolyfillsHashSettingString settingsString -ChunkHashSettingString settingsString
param( 
...
$updateWebAppSettingsScript = "$BuildSourcesDirectory\scripts\utility\update-webapp-settings.ps1"

Write-Host "Begining frontend deployment to WebAppName : '$WebAppName' WebSlotName '$WebSlotName'"
...
$distFolder = "$BuildSourcesDirectory/src/$FrontendProjectName/dist"
...
Restart-AzureRmWebAppSlot -Name $WebAppName -Slot $WebSlotName -ResourceGroupName $ResourceGroup